<script>
    ...
    export let cs, clipPath, scatterAnnimateClass, scatterPointHoverClass, colourScale, showTT, dataFilter, animationTimes;
    ...
    $: if(dataFilter){
        ...
        data = cs.data.filter(dataFilter);
...
{#each data as d}
        <circle
            ...
            in:fade="{{duration: animationTimes.inTime,
            easing: cubicInOut,
            delay: animationTimes.inDelay}}"
            out:fade="{{duration: animationTimes.outTime,
                delay: animationTimes.outDelay,
            easing: cubicInOut}}"
...
<style>
	...
	.scatter-point-animated {
		transition: fill 1s ease, stroke-width 1s ease, stroke 1s ease, cx 2s, cy 2s;
		/* -webkit-mask-image: url(paper_texture.png);
        mask-image: url(paper_texture.png); */
	}
